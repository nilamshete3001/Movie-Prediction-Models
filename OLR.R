...
model1 = update(model, method = "logistic", Hess = TRUE)#Lowest
summary(update(model, method = "probit", Hess = TRUE), digits = 3) #4145.527
summary(update(model, method = "loglog", Hess = TRUE), digits = 3) #4145.527

model2 <- stepAIC(model1, ~.^2)
polr1 = polr(formula = Category ~ movie_sequel + creative_type + source + 
               ...
               creative_type:production_method + movie_sequel:source + production_method:movie_board_rating_display_name, 
             data = updatedData, Hess = TRUE, method = "logistic")
summary(polr1)
mmodel1 <- polr1$model
...
model3 = polr(updatedData$Category.new ~ . - Category, data = updatedData, Hess = TRUE, method = "logistic") #2512.727 
...
model4 = polr(Category.new ~ movie_sequel + creative_type + 
              ...
              data = updatedData, Hess = TRUE, method = "logistic", control = list(maxit = 5000))
summary(model4)
...
model5 = polr(updatedData$Category.new2 ~ . - Category -Category.new, method = "logistic", data = updatedData, Hess = TRUE) #2161 
summary(update(model5, method = "probit", Hess = TRUE), digits = 3)
...
model6 = polr(formula = Category.new2 ~ movie_sequel + creative_type + 
                ...
                creative_type:production_method + movie_sequel:movie_board_rating_display_name+ production_method:movie_board_rating_display_name, data = updatedData, 
                        Hess = TRUE, method = "logistic")

summary(model6)
mmodel3 <- model6$model
mmodel3$modelp1 <- predict(model6)
